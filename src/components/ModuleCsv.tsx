...
import { heiConfig } from '../contexts/heiConfig';
  ...
  const handleFileChange = (newInputValue) => {
    ...
    if (fileExtension === 'csv' || fileExtension === 'xls' || fileExtension === 'xlsx') {
      ...
      reader.onload = (e) => {
        ...
        setAlasqlQueryBefore(`SELECT ROWNUM() AS id, CASE WHEN [partner_1_ec] = "${heiConfig.thisHeiEC}" THEN [partner_2_ec] ELSE [partner_1_ec] END AS CSVTH_ERASMUS_CODE, CASE WHEN [partner_1_hei_name] = "${heiConfig.thisHeiNameCsv}" THEN [partner_2_hei_name] ELSE [partner_1_hei_name] END AS CSVTH_INSTITUTION_NAME, CASE WHEN [coop_cond_type] = "staff_teachers" AND [coop_cond_sending_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_OUTGOING_STA" WHEN [coop_cond_type] = "staff_teachers" AND [coop_cond_sending_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_INCOMING_STA" WHEN [coop_cond_type] = "staff_training" AND [coop_cond_sending_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_OUTGOING_STT" WHEN [coop_cond_type] = "staff_training" AND [coop_cond_sending_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_INCOMING_STT" WHEN [coop_cond_type] = "student_studies" AND [coop_cond_sending_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_OUTGOING_SMS" WHEN [coop_cond_type] = "student_studies" AND [coop_cond_sending_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_INCOMING_SMS" WHEN [coop_cond_type] = "student_traineeship" AND [coop_cond_sending_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_OUTGOING_SMT" WHEN [coop_cond_type] = "student_traineeship" AND [coop_cond_sending_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_INCOMING_SMT" END AS CSVTH_MOBILITY_TYPE, CASE WHEN [coop_cond_eqf] IS NOT NULL AND [coop_cond_eqf] != "" THEN CAST([coop_cond_eqf] AS STRING) WHEN ([coop_cond_eqf] IS NULL OR [coop_cond_eqf] = "") AND [coop_cond_type] LIKE "staff%" THEN "CSVTD_NOT_APPLICABLE" ELSE "CSVTD_NULL" END AS CSVTH_EQF, CASE WHEN [coop_cond_blended_mobility] = "YES" THEN "CSVTD_YES" WHEN [coop_cond_blended_mobility] = "NO" THEN "CSVTD_NO" ELSE "CSVTD_NOT_APPLICABLE" END AS CSVTH_BLENDED, [coop_cond_total_people] AS CSVTH_NUMBER_OF_MOBILITIES, CASE WHEN [iia_status] = "approved-by-all" THEN "CSVTD_APPROVED_BY_ALL" WHEN [iia_status] = "approved" AND [partner_1_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_WAITING_FOR_THEIR_SIGNATURE" WHEN [iia_status] = "approved" AND [partner_1_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_WAITING_FOR_OUR_SIGNATURE" WHEN [iia_status] = "submitted" AND [partner_1_hei_id] = "${heiConfig.thisHeiDomain}" THEN "CSVTD_BEING_VERIFIED_BY_THEM" WHEN [iia_status] = "submitted" AND [partner_1_hei_id] != "${heiConfig.thisHeiDomain}" THEN "CSVTD_BEING_VERIFIED_BY_US" WHEN [iia_status] = "draft" THEN "CSVTD_DRAFT" END AS CSVTH_STATUS, CASE WHEN [coop_cond_subject_area] IS NULL OR [coop_cond_subject_area] = "" THEN "CSVTD_NULL" WHEN [coop_cond_subject_area] LIKE "%,%" OR [coop_cond_subject_area] LIKE "0%" THEN [coop_cond_subject_area] WHEN CAST([coop_cond_subject_area] AS INT) IN (1000, 1010, 1011, 1012, 1013, 1014, 1015, 1019, 1020, 1021, 1022, 1029, 1030, 1031, 1032, 1039, 1040, 1041, 1049, 1088) THEN [coop_cond_subject_area] ELSE "0" + [coop_cond_subject_area] END AS CSVTH_SUBJECT_AREA, CASE WHEN [coop_cond_subject_area_clarification] IS NULL OR [coop_cond_subject_area_clarification] = "" THEN "CSVTD_NULL" ELSE [coop_cond_subject_area_clarification] END AS CSVTH_SUBJECT_AREA_DESCRIPTION, CASE WHEN [coop_cond_language] IS NULL OR [coop_cond_language] = "" THEN "CSVTD_NULL" ELSE [coop_cond_language] END AS CSVTH_LANGUAGE_REQUIREMENTS, [coop_cond_academic_year_start] AS CSVTH_FROM, [coop_cond_academic_year_end] AS CSVTH_TO, CASE WHEN [partner_1_hei_id] = "${heiConfig.thisHeiDomain}" THEN [partner_2_hei_id] ELSE [partner_1_hei_id] END AS CSVTH_PARTNER_WWW
`);
